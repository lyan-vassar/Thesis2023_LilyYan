...
// helper method: check for duplicates in an array
function noDuplicates(arr) {
    var currentValues = [];
    for (ctr = 0; ctr < arr.length; ctr++) {
        if (arr[ctr] in currentValues) return false;
        currentValues.push(arr[ctr]);
    }
    return true;
}

// helper method: check if two arrays are equal
function arrEqual(arr1, arr2) {
    if (arr1.length != arr2.length) return false;
    for (ctr = 0; ctr < arr1.length; ctr++) {
        if (arr1[ctr] != arr2[ctr]) return false;
    }
